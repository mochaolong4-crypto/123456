...
<body>
    ...
    <script type="module">
        ...
        function setupPostProcessing() {
            // 修改：大幅降低Bloom（发光）效果的强度，让画面更干净
            const bloom = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.4, 0.85);
            bloom.threshold = 0.85; // 提高阈值，只有很亮的地方才发光
            bloom.strength = 0.15;  // 降低强度，去除了“梦幻滤镜”般的模糊感
            bloom.radius = 0.1;     // 减小光晕扩散范围，让物体边缘更清晰
        ...
        function createMaterials() {
            ...
            // 修改：降低星星的自发光强度，避免刺眼
            matLib.starGold = new THREE.MeshStandardMaterial({ color: 0xffdd88, emissive: 0xffaa00, emissiveIntensity: 1.0, metalness: 1.0, roughness: 0 });
            ...
            matLib.frozenCyan = new THREE.MeshPhysicalMaterial({
                ...
                emissive: CONFIG.colors.iceCyan, emissiveIntensity: 1.0,
                clearcoat: 1.0
            });
            matLib.frozenMagenta = new THREE.MeshPhysicalMaterial({
                color: CONFIG.colors.iceMagenta, metalness: 0.5, roughness: 0.2,
                emissive: CONFIG.colors.iceMagenta, emissiveIntensity: 0.8,
                clearcoat: 1.0
            });
            matLib.starIce = new THREE.MeshPhysicalMaterial({ 
                color: CONFIG.colors.iceCyan, emissive: 0xffffff, emissiveIntensity: 1.5, 
                metalness: 0.2, roughness: 0, transmission: 0.6, thickness: 3.0 
            });
            matLib.snow = new THREE.MeshStandardMaterial({ color: 0xffffff, metalness: 0.1, roughness: 0.8, emissive: 0xaaaaaa, emissiveIntensity: 0.5 });
            ...
            matLib.starBarbie = new THREE.MeshStandardMaterial({ color: CONFIG.colors.barbieHot, emissive: 0xff0088, emissiveIntensity: 2.0, metalness: 0.5, roughness: 0.1 });
        ...
        class Particle {
            ...
            update(dt, time, mode, focusTargetMesh, invMatrix) {
                ...
                if (this.hasEmissive && mode === 'TREE' && !this.isDust) {
                    ...
                    // 修改：降低闪烁时的最大亮度
                    const maxEmit = (STATE.currentThemeIndex === 1) ? 1.5 : 1.2;
                    this.mesh.material.emissiveIntensity = blink > 0.5 ? (0.5 + (blink - 0.5) * maxEmit) : 0.4;
                }
        ...
        function switchTheme(themeIndex) {
            ...
            // 修改：限制各主题下的最大曝光度
            renderer.toneMappingExposure = (themeIndex === 1) ? 1.0 : (themeIndex === 2 ? 0.7 : 0.8);
        ...
        function setupEvents() {
            ...
            btnSwitch.onclick = (e) => {
                ...
                btnSwitch.textContent = STATE.isGestureSwitchEnabled ? "颜色切换" : "颜色切换";
                if (STATE.isGestureSwitchEnabled) {
                    btnSwitch.classList.remove('disabled');
                } else {
